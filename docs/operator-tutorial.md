...
The full controller is in [examples/pod-terminator-operator](https://github.com/snebel29/kooper/tree/master/examples/pod-terminator-operator).
...
in `types` is our [PodTerminator Go object](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/apis/chaos/v1alpha1/types.go) that describes de API and in `register.go` files there is data to register this types in kubernetes client.

With these files kubernetes code-generator will generate the required code for the [clients](https://github.com/snebel29/kooper/tree/master/examples/pod-terminator-operator/client/k8s/clientset) and deepcopy methods ([deepcopy](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/apis/chaos/v1alpha1/zz_generated.deepcopy.go) methods are required by all the kubernetes objects). You can see how its used in the [Makefile](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/Makefile). This will generate all the boilerplate code that is the same in all the kubernetes objects (crds and not crds).
...
We will not explain the logic of this service as is out of the scope of this operator tutorial. But you can take a look [here]((https://github.com/snebel29/kooper/tree/master/examples/pod-terminator-operator/service/chaos) if you think is interesting.
...
This can be found in [operator/config.go](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/operator/config.go)
```
type Config struct {
...
This can be found in [operator/crd.go](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/operator/crd.go).

For the initialization kooper [CRD client](https://github.com/snebel29/kooper/blob/master/client/crd/crd.go) is used.
...
This can be found in [operator/handler.go](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/operator/handler.go).
...
This can be found in [operator/factory.go](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/operator/factory.go).
```golang
...
After all these steps, a fully operational chaos operator is ready to destroy pods. You can check the [main](https://github.com/snebel29/kooper/blob/master/examples/pod-terminator-operator/cmd/main.go) where the operator is instantiated.
